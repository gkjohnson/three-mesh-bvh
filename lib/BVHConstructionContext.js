...
function computeBounds( geo ) {
	...
	const result = new Float32Array( triCount * 6 );
	...
	for ( let tri = 0; tri < triCount; tri ++ ) {
		...
		for ( let el = 0; el < 3; el ++ ) {
			...
			result[ tri * 6 + el * 2 ] = Math.min( a, b, c );
			result[ tri * 6 + el * 2 + 1 ] = Math.max( a, b, c );
...
function computeCentroids( geo ) {

	const triCount = getTriangleCount ( geo );
	const result = new Float32Array( triCount * 3 );
...
export default class BVHConstructionContext {
	...
	constructor( geo, options ) {
		...
		this.centroids = computeCentroids( geo );
	...
	shrinkBoundsTo( offset, count, target ) {
		...
		target[ 0 ] = Math.max( minx, target[ 0 ] );
		target[ 1 ] = Math.max( miny, target[ 1 ] );
		target[ 2 ] = Math.max( minz, target[ 2 ] );

		target[ 3 ] = Math.min( maxx, target[ 3 ] );
		target[ 4 ] = Math.min( maxy, target[ 4 ] );
		target[ 5 ] = Math.min( maxz, target[ 5 ] );
