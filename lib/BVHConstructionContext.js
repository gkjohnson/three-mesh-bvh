...
export default class BVHConstructionContext {
	...
	constructor( geo, options ) {
		...
		this.tris = new Array( geo.index ? ( geo.index.count / 3 ) : ( geo.attributes.position.count / 3 ) );
	...
	computeBounds() {
		...
		const result = new Float32Array( tris.length * 6 );
	...
	computeCentroids() {
		...
		const geo = this.geo;
		const tris = this.tris;
		const result = new Float32Array( tris.length * 3 );
