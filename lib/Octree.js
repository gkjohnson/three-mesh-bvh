...
class OctreeNode {
    ...
    raycast(raycaster, intersects) {
        ...
        this._objects.forEach(o => {
            if (raycastDedupeMap.has(o)) return;
    ...
    raycastFirst(raycaster) {
        ...
        let closest = arr.pop() || null;
        if (this._nodes) {

            this._nodes.forEach((n, i) => {
                this._sortArray[i].node = n;
                ...
                if (n) {
                    const dist = 
                        tempvec
                            ...
                            .length() * n._center.dot(raycaster.ray.origin);
                    this._sortArray[i].dist = dist;            
                } else {
                    this._sortArray[i].dist = Infinity;
                }
            });
            this._sortArray.sort((a, b) => a.dist === b.dist ? 0 : a.dist - b.dist)

            for (let i = 0; i < this._sortArray.length; i ++) {
                const inf = this._sortArray[i];
                if (!inf.node || inf.dist === Infinity) break;
