...
class OctreeNode extends OctreeObject {
    ...
    raycast(raycaster, ray) {
        if (this._objects.length) {
            this._objects.forEach(m => {
                m.material = new THREE.MeshBasicMaterial({ color: 0x00ff00 })
            });
            return;
        }
        ...
        let delta;
        let flag = 0;
        delta = ray.origin.x - this._center.x;
        flag |= delta > 0 ? (1 << 0) : 0;
        raycastarr[0].flag = X_FLAG;
        raycastarr[0].dist = ray.direction.x === 0 ? -Infinity : delta / -ray.direction.x;

        delta = ray.origin.y - this._center.y;
        flag |= delta > 0 ? (1 << 1) : 0;
        raycastarr[1].flag = Y_FLAG;
        raycastarr[1].dist = ray.direction.y === 0 ? -Infinity : delta / -ray.direction.y;
        
        delta = ray.origin.z - this._center.z;
        flag |= delta > 0 ? (1 << 2) : 0;
        raycastarr[2].flag = Z_FLAG;
        raycastarr[2].dist = ray.direction.z === 0 ? -Infinity : delta / -ray.direction.z;

        
        if (this._nodes[flag]) this._nodes[flag].raycast(raycaster, ray);

        raycastarr.sort((a, b) => a.dist - b.dist);

        // account for passing directly through the intersection
        // of two planes
        // if (raycastarr[0].dist === raycastarr[1].dist) raycastarr[0].dist = -Infinity;
        // if (raycastarr[1].dist === raycastarr[2].dist) raycastarr[1].dist = -Infinity;

        raycastarr.forEach(inf => {
            // flip the bit for our current flag
            flag = ((~flag) & inf.flag) | (flag & (~inf.flag));

            if (inf.dist <= 0) return;
    ...
    _tryConsolidate() {
        ...
        this._nodes.forEach((n, i) => {
            ...
            if (n._objects.length === 0) {
                this._nodes[i] = null;
            }
            else inNodes += n._objects.length;
        })
