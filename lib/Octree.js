...
class OctreeNode {
	...
	raycastFirst( raycaster ) {
		...
		// if > or < center value
		// TODO: This doesn't always seem to yield the right
		if ( ! this.intersectsRay( raycaster.ray ) ) return;
		...
		if ( this._nodes ) {
			...
			for ( let i = 0, l = nodes.length; i < l; i ++ ) {
				...
				if ( n ) {
					...
					const sphere = n._sphere;
					tempvec.copy( sphere.center ).sub( raycaster.ray.origin );

					const dist = Math.max( sphere.distanceToPoint( raycaster.ray.origin ), 0 ) * tempvec.dot( raycaster.ray.direction );
					inf.dist = dist;
			...
			for ( let i = 0, l = sortArr.length; i < l; i ++ ) {
				...
				const inf = sortArr[ i ];
				if ( ! inf.node || inf.dist < 0 ) continue;
