...
// TODO
// - Optimize insert and update
// - Implement other casts
// - Test animated / constantly re-inserted nodes for update time
// - Create a callback Object3D that can be used to keep the tree in sync
// - Test tree expansion / shrinking
...
class OctreeNode {
	...
	raycast( raycaster, intersects = [] ) {
		...
		// TODO: Reuse the ray traversal here
	...
	_addPending( o ) {
		...
		if (
			this._root._maxDepth === this._depth
			|| this._nodes === null
		) {
...
class Octree {
	...
	constructor( width = 64, maxDepth = Infinity, maxObjects = 4 ) {
	...
	_runAction( o, ac ) {
		...
		const add = o => {
			...
			// TODO: This bounding sphere isn't correct and doesn't
			// update the position appropriately, so rotating a parent
			// object does not update this value. We need to manually
			// recomputed this on insert here
			const sp = o.boundingSphere.clone();
			this._objectMap.set( o, sp );

			// TODO: This needs to account for the global object position
		...
		const remove = ( o, sp ) => {
			...
			// TODO: if this is slow then we could iteratively
			// push them down in to the next octant layer deep
			// and only update when the node is being traversed
			this.root._search( sp, n => {

				const pindex = n._pendingInserts.indexOf( o );
				const oindex = n._objects.indexOf( o );

				if ( pindex !== - 1 ) {

					n._pendingInserts.splice( pindex, 1 );

				}

				if ( oindex !== - 1 ) {

					n._objects.splice( oindex, 1 );
