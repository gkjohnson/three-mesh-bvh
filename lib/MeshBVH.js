...
import {
	...
	getBounds, getCentroids, shrinkBoundsTo } from './GeometryUtilities.js';
...
class MeshBVH extends MeshBVHNode {
	...
	_buildTree( geo, options ) {
		...
		const bounds = getBounds( origTris, geo, vertexElem );
		...
		const splitStrategy = ( bb, tris, centroids ) => {
		...
		const createNode = ( tris, bb, newNode, depth = 0 ) => {
			...
			if ( tris.length <= options.maxLeafNodes ) {
