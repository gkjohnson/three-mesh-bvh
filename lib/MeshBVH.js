...
import {
    getBufferGeometryVertexElem, getGeometryVertexElem, getLongestEdgeIndex,
    getAverage, shrinkBoundsTo, shrinkSphereTo, intersectTri, intersectTris } from './GeometryUtilities.js'
...
class MeshBVHNode {
    ...
    collectCandidates(origray) {
        ...
        recurse(this, origray);

        return candidates;
    }

    raycastAll(mesh, raycaster, origray, intersects) {

        const recurse = (node, ray) => {
            if (!ray.intersectsSphere(node.boundingSphere) || !ray.intersectsBox(node.boundingBox)) return;
     
            if (node.tris) intersectTris(mesh, node.geometry, raycaster, ray, node.tris, intersects);
...
class MeshBVH {
    ...
    /* Public API */
    collectCandidates(...args) {
        return this._root.collectCandidates(...args);
    }

    raycastAll(...args) {
        return this._root.raycastAll(...args);
    }
