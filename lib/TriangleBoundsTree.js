    ...
    _initBufferGeometry(geo) {
        ...
        const recurse = tris => {
            ...
            for (let i = 0; i < tris.length; i ++) {
                ...
                else if (inLeft) left.push(tri);
                else if (inRight) right.push(tri);
            }

            if (shared.length / tris.length > maxMatchingTriangles) {
                node.tris = tris;
            } else {
                if (left.length) node.children.push(recurse(left));
                if (right.length) node.children.push(recurse(right));
                if (shared.length) node.children.push(recurse(shared));
            }
