import{S as R,q as b,V as y,W as Y,c as $,D as K,A as Q,P as X,ap as J,b as Z,M as ee,g as te,a as oe,R as B,F as M,H as L,L as S,aa as ne,aq as ie,Y as re,t as ae,ar as se}from"./ExtendedTriangle-CiRTBY2J.js";import{G as ce}from"./GLTFLoader-BQkAV-ec.js";import{F}from"./Pass-VgeIyLfL.js";import{O as de}from"./OrbitControls-V1tyEqOf.js";import{g as le}from"./lil-gui.module.min-jESndyO-.js";import{S as ue}from"./stats.min-GTpOrGrX.js";import{M as ve}from"./MeshBVHUniformStruct-CnLrNbMU.js";import{c as fe,b as me,a as pe}from"./bvh_struct_definitions.glsl-SZg5BxSQ.js";import{M as xe}from"./meshopt_decoder.module-Cf1-17OU.js";import{G as he}from"./GenerateMeshBVHWorker-CLw2P6vs.js";import{S as ye}from"./StaticGeometryGenerator-B6C48lkG.js";import"./BufferGeometryUtils-BlCRhshM.js";import"./_commonjsHelpers-Cpj98o6Y.js";import"./MeshBVH-DTdncBTW.js";const ge=`
...
`;class be extends R{constructor(o){super({defines:{USE_SHADER_RAYCAST:window.location.hash.includes("USE_SHADER_RAYCAST")?1:0},uniforms:{matrix:{value:new b},zValue:{value:0},bvh:{value:new ve}},vertexShader:`
			...
			`}),this.setValues(o)}}class we extends R{constructor(o){super({defines:{DISPLAY_GRID:0},uniforms:{sdfTex:{value:null},layer:{value:0},layers:{value:0}},vertexShader:`
			...
			`}),this.setValues(o)}}class Te extends R{constructor(o){super({defines:{MAX_STEPS:500,SURFACE_EPSILON:.001},uniforms:{surface:{value:0},sdfTex:{value:null},normalStep:{value:new y},projectionInverse:{value:new b},sdfTransformInverse:{value:new b}},vertexShader:`
			...
			`}),this.setValues(o)}}const t={gpuGeneration:!0,resolution:75,margin:.2,regenerate:()=>N(),mode:"raymarching",layer:0,surface:.1};let i,a,m,h,z,c,V,I,v,n,P,g,A,u,U;const G=new b;De();k();function De(){V=document.getElementById("output"),i=new Y({antialias:!0}),i.setPixelRatio(window.devicePixelRatio),i.setSize(window.innerWidth,window.innerHeight),i.setClearColor(0,0),document.body.appendChild(i.domElement),m=new $;const e=new K(16777215,1);e.position.set(1,1,1),m.add(e),m.add(new Q(16777215,.2)),a=new X(75,window.innerWidth/window.innerHeight,.1,50),a.position.set(1,1,2),a.far=100,a.updateProjectionMatrix(),c=new J(new Z),m.add(c),new de(a,i.domElement),z=new ue,document.body.appendChild(z.dom),g=new F(new be),A=new F(new we),u=new F(new Te),U=new he,new ce().setMeshoptDecoder(xe).loadAsync("https://raw.githubusercontent.com/gkjohnson/3d-demo-data/main/models/stanford-bunny/bunny.glb").then(o=>{o.scene.updateMatrixWorld(!0);const r=new ye(o.scene);return r.attributes=["position","normal"],r.useGroups=!1,v=r.generate().center(),U.generate(v,{maxLeafTris:1})}).then(o=>{I=o,P=new ee(v,new te),m.add(P),N()}),C(),window.addEventListener("resize",function(){a.aspect=window.innerWidth/window.innerHeight,a.updateProjectionMatrix(),i.setSize(window.innerWidth,window.innerHeight)},!1)}function C(){h&&h.destroy(),t.layer=Math.min(t.resolution,t.layer),h=new le;const e=h.addFolder("generation");e.add(t,"gpuGeneration"),e.add(t,"resolution",10,200,1),e.add(t,"margin",0,1),e.add(t,"regenerate");const o=h.addFolder("display");o.add(t,"mode",["geometry","raymarching","layer","grid layers"]).onChange(()=>{C()}),t.mode==="layer"&&o.add(t,"layer",0,t.resolution,1),t.mode==="raymarching"&&o.add(t,"surface",-.2,.5)}function N(){const e=t.resolution,o=new b,r=new y,_=new se,p=new y;v.boundingBox.getCenter(r),p.subVectors(v.boundingBox.max,v.boundingBox.min),p.x+=2*t.margin,p.y+=2*t.margin,p.z+=2*t.margin,o.compose(r,_,p),G.copy(o).invert(),c.box.copy(v.boundingBox),c.box.min.x-=t.margin,c.box.min.y-=t.margin,c.box.min.z-=t.margin,c.box.max.x+=t.margin,c.box.max.y+=t.margin,c.box.max.z+=t.margin,n&&n.dispose();const x=1/e,w=.5*x,E=window.performance.now();if(t.gpuGeneration){const f=i.extensions.get("OES_texture_float_linear");n=new oe(e,e,e),n.texture.format=B,n.texture.type=f?M:L,n.texture.minFilter=S,n.texture.magFilter=S,i.initRenderTarget(n),g.material.uniforms.bvh.value.updateFrom(I),g.material.uniforms.matrix.value.copy(o);const l=new y,d=new ne(e,e);d.texture.format=B,d.texture.type=f?M:L;for(let s=0;s<e;s++)g.material.uniforms.zValue.value=s*x+w,i.setRenderTarget(d),g.render(i),l.z=s,i.copyTextureToTexture(d.texture,n.texture,null,l);i.readRenderTargetPixels(d,0,0,1,1,new Float32Array(4)),i.setRenderTarget(null),d.dispose()}else{n=new ie(new Float32Array(e**3),e,e,e),n.format=B,n.type=M,n.minFilter=S,n.magFilter=S,n.needsUpdate=!0;const f=new y,l=new re,d={};for(let s=0;s<e;s++)for(let T=0;T<e;T++)for(let D=0;D<e;D++){f.set(w+s*x-.5,w+T*x-.5,w+D*x-.5).applyMatrix4(o);const O=s+T*e+D*e*e,q=I.closestPointToPoint(f,d).distance;l.origin.copy(f),l.direction.set(0,0,1);const H=I.raycastFirst(l,ae),j=H&&H.face.normal.dot(l.direction)>0;n.image.data[O]=j?-q:q}}const W=window.performance.now()-E;V.innerText=`${W.toFixed(2)}ms`,C()}function k(){if(z.update(),requestAnimationFrame(k),n){if(t.mode==="geometry")i.render(m,a);else if(t.mode==="layer"||t.mode==="grid layers"){let e;const o=A.material;n.isData3DTexture?(o.uniforms.layer.value=t.layer/n.image.width,o.uniforms.sdfTex.value=n,e=n):(o.uniforms.layer.value=t.layer/n.width,o.uniforms.sdfTex.value=n.texture,e=n.texture),o.uniforms.layers.value=e.image.width;const r=t.mode==="layer"?0:1;r!==o.defines.DISPLAY_GRID&&(o.defines.DISPLAY_GRID=r,o.needsUpdate=!0),A.render(i)}else if(t.mode==="raymarching"){a.updateMatrixWorld(),P.updateMatrixWorld();let e;n.isData3DTexture?e=n:e=n.texture;const{width:o,depth:r,height:_}=e.image;u.material.uniforms.sdfTex.value=e,u.material.uniforms.normalStep.value.set(1/o,1/_,1/r),u.material.uniforms.surface.value=t.surface,u.material.uniforms.projectionInverse.value.copy(a.projectionMatrixInverse),u.material.uniforms.sdfTransformInverse.value.copy(P.matrixWorld).invert().premultiply(G).multiply(a.matrixWorld),u.render(i)}}else return}
