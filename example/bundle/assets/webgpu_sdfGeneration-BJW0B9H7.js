import{n as x,V as B,c as K,D as Q,A as X,P as Y,ap as Z,b as $,M as J,g as ee,aq as te,R as oe,F as re,aR as _}from"./ExtendedTriangle-DttJMGjs.js";import{w as s,b as q,k as C,N as W,m as H,o as F,p as A,q as O,u as c,x as k,W as ne,S as I,y as ae,s as P,z as se,A as ie}from"./common_functions.wgsl-CjdvKpsU.js";import{M as de}from"./meshopt_decoder.module-j6OW_3Rk.js";import{G as le}from"./GLTFLoader-DLQrcS9x.js";import{F as z}from"./Pass-C67NYBa3.js";import{O as ce}from"./OrbitControls--aO4oMeG.js";import{g as fe}from"./lil-gui.module.min-BH_YJbPT.js";import{S as me}from"./stats.min-DbzWzcqd.js";import{G as ue}from"./GenerateMeshBVHWorker-Bnlb02Pc.js";import{S as pe}from"./StaticGeometryGenerator-Bt7PWAl_.js";import"./BufferGeometryUtils-BVewXkEy.js";import"./_commonjsHelpers-CqkleIqs.js";import"./MeshBVH-CIpiS6_0.js";const E=C(`
...
`,[q,E,ge,he]);class we extends W{constructor(e){super();const d={surface:c(0),normalStep:c(new B),projectionInverse:c(new x),sdfTransformInverse:c(new x),sdfTransform:c(new x),uv:A(O()),sdf_sampler:F(e),sdf:H(e)},f=s(`
		...
		`,[f]);this.fragmentNode=a(d);const m={position:k},u=s(`

			fn noop(position: vec4f) -> vec4f {
				return position;
			}

		`);this.vertexNode=u(m)}}class Se extends W{constructor(e){super();const d=s(`
		...
		`),f={layer:c(0),grid_mode:c(!1),uv:A(O()),sdf_sampler:F(e),sdf:H(e)};let a=s(`
			fn layer(
				layer: u32,
				grid_mode: bool,
		...
		`,[d]);this.fragmentNode=a(f);const m={position:k},u=s(`

			fn noop(position: vec4f) -> vec4f {
				return position;
			}

		`);this.vertexNode=u(m)}}const T=[4,4,4],t={resolution:75,margin:.2,regenerate:()=>L(),mode:"raymarching",layer:0,surface:.1};let n,i,g,b,R,l,V,M,v,r,h,w,S,D,y;const G=new x;Te().then(j);async function Te(){if(!await navigator.gpu.requestAdapter())throw document.body.appendChild(_e()),new Error("No WebGPU support");V=document.getElementById("output"),n=new ne,n.setPixelRatio(window.devicePixelRatio),n.setSize(window.innerWidth,window.innerHeight),n.setClearColor(0,0),document.body.appendChild(n.domElement),await n.init(),g=new K;const o=new Q(16777215,1);o.position.set(1,1,1),g.add(o),g.add(new X(16777215,.2)),i=new Y(75,window.innerWidth/window.innerHeight,.1,50),i.position.set(1,1,2),i.far=100,i.updateProjectionMatrix(),l=new Z(new $),g.add(l),new ce(i,n.domElement),R=new me,document.body.appendChild(R.dom),D=new ue;const e=await new le().setMeshoptDecoder(de).loadAsync("https://raw.githubusercontent.com/gkjohnson/3d-demo-data/main/models/stanford-bunny/bunny.glb");e.scene.updateMatrixWorld(!0);const d=new pe(e.scene);d.attributes=["position","normal"],d.useGroups=!1,v=d.generate().center(),M=await D.generate(v,{maxLeafTris:1}),h=new J(v,new ee),g.add(h);const f=new I(h.geometry.index.array,3),a=new I(h.geometry.attributes.position.array,3),m=new I(new Float32Array(M._roots[0]),8),u={matrix:c(new x),dim:c(0),bvh_index:P(f,"uvec3",f.count).toReadOnly(),bvh_position:P(a,"vec3",a.count).toReadOnly(),bvh:P(m,"BVHNode",m.count).toReadOnly(),globalId:se,output:ae(r)};y=s(`
